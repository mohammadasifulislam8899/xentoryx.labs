...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-[100dvh] h-[100dvh] w-full flex flex-col justify-between overflow-x-clip bg-[var(--bg-primary)] text-[var(--text-primary)] px-4 sm:px-6 md:px-10 select-none transition-colors duration-300">
      ...
      {/* 1. Top Navbar */}
      <FadeIn delay={0} y={-20} className="w-full pt-4 sm:pt-6 md:pt-8 z-40">
        <nav className="w-full flex items-center justify-between">
          ...
          <div className="flex sm:hidden items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-full border border-[var(--border-color)] bg-[var(--bg-secondary)] text-[var(--text-primary)] cursor-pointer"
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden absolute top-full left-0 right-0 mt-2 mx-4 p-5 rounded-3xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-2xl flex flex-col gap-4 z-50 animate-in fade-in slide-in-from-top-4 duration-200">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
      ...
      {/* 2. Hero Heading */}
      <div className="w-full overflow-hidden text-center z-0 mt-4 sm:mt-2 md:-mt-4">
        <FadeIn delay={0.15} y={40} className="w-full">
          <h1 className="hero-heading font-black uppercase tracking-tight leading-none whitespace-nowrap w-full text-[13vw] sm:text-[14vw] md:text-[16vw] lg:text-[17.5vw]">
            Hi, i&apos;m asif
          </h1>
        </FadeIn>
      </div>

      {/* 3. Hero Portrait: Responsive Scaled Portrait */}
      <div className="absolute left-1/2 -translate-x-1/2 bottom-0 z-10 w-full max-w-[1900px] h-[58vh] sm:h-[75vh] md:h-[95vh] lg:h-[115vh] xl:h-[130vh] pointer-events-none flex justify-center items-end overflow-visible">
      ...
      <div className="w-full flex justify-between items-end pb-5 sm:pb-8 md:pb-10 z-30 pointer-events-auto gap-3">
        ...
        <FadeIn delay={0.35} y={20}>
          <p
            className="text-[var(--text-primary)] font-light uppercase tracking-wide leading-snug text-[10px] sm:text-xs md:text-sm lg:text-base max-w-[150px] sm:max-w-[200px] md:max-w-[260px]"
          >
